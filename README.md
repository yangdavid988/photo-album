...
JPEG images are served from a **microSD card** (FatFS over VFS, hot-pluggable) or — when no readable card is present — from **flash as C arrays** (XIP), and decoded by the **MJPEG hardware decoder** (Vivante GC/HX170 + PP post-processor), then displayed fullscreen through an LVGL 9.3 canvas.
...
| SD JPEG staging buffer (max photo size) | PSRAM `.psram_heap.start` | 2 MB |
...
**SD card**: put the JPEGs in a `JPG/` folder on a FAT32 card, formatted without a
partition table (an MBR-partitioned card is reported unreadable). Files are picked up by
their `0xFF 0xD8 0xFF` header, not by extension, and any file over the 2 MB staging buffer
is skipped. The card is mounted read-only; hot-plug runs off the card-detect pin, so
swapping albums needs no reset or re-flash.
